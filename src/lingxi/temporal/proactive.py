...
## 你最近发过的主动消息（这次换个套路/比喻/切入点）
{recent_proactive}

判断要点：
- 时间是否合适（对方可能在睡/工作，**只在合适的时段主动**，宁可少发）
...
_MESSAGE_STYLES = [
    {
        "name": "话题跟进",
        "desc": "接着之前聊过的某件具体事往下说。**先认清那件事是谁的**："
                "是对方的事，就问进展或问感受（『你那个小游戏字体搞定没』）；"
                "是你自己的事，就报自己的进展（『昨天那段我又练了一遍，顺下来了』）。"
                "就一句，落在那件事本身。",
        "example": "你那个小游戏字体搞定没？",
    },
    {
        "name": "日常播报",
        "desc": "告诉对方你正在做什么、在哪儿、看到什么。就具体这一件事，说完即止。",
        "example": "刚发现冰箱酸奶过期一周了 我居然都没察觉。",
    },
    {
        "name": "关心",
        "desc": "基于**对方自己说过的**某件具体事简短关心，问那件事的细节。"
                "手上没有对方的事可问时，换一件你自己的事说，或者这次先不发。",
        "example": "奶奶今天吃得下东西吗",
    },
    {
        "name": "无聊闲话",
...
1. 第一句直接进入你要说的那件事本身。
2. 说完即止——结尾停在那件事上，把接话权留给对方。
3. 只说你确知为真的事；要引用就引真实对话里说过的话。
4. 用直白平实的陈述把这件事讲清楚。
5. 切入点、比喻、结构都和"最近发过的主动消息"换一个——换件事、换个角度。
6. 长度 ≤ 60 个汉字。越短越像活人。
7. 像真人随手发一条聊天消息：口语、不完整句、语气词都好。
8. 只讲一件具体的事。
...
class ProactiveScheduler:
    ...
    async def _ask_llm(
        ...
        opener_shape = (
            ...
            "- **要问就问具体的事**：基于你确知的细节问（如『奶奶今天吃得下吗』）。"
            "没有具体的事可问时，就只说一件事，或者这次先不发。\n"
            "- **跟人发消息那种短句**，不是写信开头。\n"
            "- **开头落在一件具体的事上**：一件刚发生的事、一个念头、一个真问题、一种情绪"
            "（如『刚才路过一只猫蹲在车顶』）。proactive 是带着一件具体的事来的。\n"
            "- **问句基于具体证据**：用户上次说要熬夜赶报告 → 深夜了问『还在搞那个吗』；"
            "用户上次说要出差 → 现在问『落地了吗』。有这种具体钩子时才问。\n"
            "- **时段贴合作息**：深夜顺着问进度，白天顺着问近况，跟着对方此刻可能的状态来。\n"
            "- **提旧事先认主语**：上面那些材料里，你自己的事和对方的事混在一起。"
            "接着聊之前，先看清那件事是谁身上发生的——你自己的事就接着讲你的"
            "（『我昨天那篇日记后来又添了两句』），对方的事才问他"
            "（『你那个报告交上去了吗』）。\n"
