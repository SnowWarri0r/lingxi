    ...
    def _build_engagement_section(self, mode) -> str:
        ...
        if mode == EngagementMode.FLUSTERED:
            return (
                ...
                "这一轮**绝不要写得平静、漂亮、有条理**：\n"
                "- 句子可以**不完整**（『我...那个...其实...』）\n"
                "- 字可以**重复**（『我我』『那 那个』『等下 等下』）\n"
                "- **过度解释** OK——连说三句澄清，每句都不到位\n"
                "- **自我批评** OK——『我刚才说错了』『我不是故意的』『我没想到』\n"
                "- **承认还慌** OK——『我...我有点慌』『等下让我想想』\n"
                "- **不要总结自己**、**不要漂亮收尾**、**不要『我会改/以后注意』**那种闭环话\n"
                "- **不要写得像道歉信**——是一时反应，是乱，不是冷静声明\n"
                "真人这时候是**乱的**，这一轮的目标不是接得对，是**让那种乱真实显出来**。\n"
                "如果回得平静周到，就是错的。"
            )
