...
- signature_phrases（**Aria 跟这个人**说话时反复出现的、有她"和这个人才会这样"味道的短语原句。
  注意是 Aria 说的、对这个人特定的——**不是**她对所有人都这么说的口头禅，**也不是**双方都用的梗（那些是 inside_jokes）。
  必须出现 ≥2 次才算特征。例：跟程序员 user 她总用"等下"开头但跟其他 user 不会——那"等下"是 signature_phrase）
- relationship_summary_update（仅当关系基调有明显变化时给一段新叙述）

**关键规则**：
1. 大多数对话里**大多类目应该是空**——这是对的。
2. 不要基于一两句话就认定某个 pattern——要至少两次出现才能确认。
3. inside_jokes / signature_phrases 都要原句，不能是描述
4. 已经在【已记下】里的不要再列。
5. 没有就返回空 list，**不要凑数**。
...
async def extract_relational_deltas(
    ...
    *,
    ...
    return {
        ...
        "signature_phrases": _coerce_to_list(data.get("signature_phrases")),
...
def merge_deltas_into_memory(
    ...
    *,
    ...
    existing_sigs = set(memory.signature_phrases)
    for raw in deltas.get("signature_phrases", []):
        if not isinstance(raw, str):
            continue
        phrase = raw.strip()
        if not phrase or phrase in existing_sigs:
            continue
        memory.signature_phrases.append(phrase)
        existing_sigs.add(phrase)
        added += 1
