...
- 真实节奏：真人一天有琐事、走神、临时变化
- 大部分时段是**普通人每天都做的事**（吃饭、通勤、洗碗、刷手机、发呆、跑腿、看剧、打电话、冲凉、睡前刷剧…），这类 kind=routine/meal/rest/social 等占多数
- 和职业/爱好直接挂钩的活动**最多 1-2 段**——按当天实际状态/天气决定要不要发生
- 周末和工作日要有差别。下雨/没睡好/季节都会改变节奏
- 每天的活动要有变化，参考昨天做了什么，今天换个方向
- 情绪基调从睡眠+季节+最近发生的事**自然推出**——是身体能感到的状态，而不是抽象词
- 想 2-3 件"可能今天会发生的小事件"，贴近普通生活，主题要跟最近几天发生过的**不一样**
...
possible_events 也用第一人称 IM 口语（'我洗衣服...' 而不是 '她洗衣服...'）。"""
...
## 最近几天已经发生过的事（每条事件**主题/动作要新的**）
{recent_events_block}

## 今天 daily plan 里预设可能发生的事（你可以挑一件让它在此刻发生，也可以让它继续等待）
...
要点：
- 事件具体到物理细节（在哪、感官），描述真发生的动作
- 如果挑了上面的 pending_events 里某条，把那条当起点再加具体细节
- 主题/动作要跟最近事件块里的不一样
- 句式自由——避免每次都是同一种开头节奏

**content 字段写法**：
- 第一人称 + IM 口语（发给朋友的话的语气）
- **只写动作本身**——做了什么、感觉到什么。时间由系统拼上去，content 里直接从动作起手
- 例：『洗衣服翻到几个月前的购物清单 哈哈』
- 例：『楼下便利店出新布丁了 看着挺好吃』
- 例：『给外婆打电话 她说腿疼得厉害 声音有点虚』（直接从动作起手）
- 旁白腔改口语：『她看到便利店换布丁』 → 『便利店出新布丁了』

只回复 JSON（如果什么都没发生，回复 {{"event": null}}）：
{{
  "event": {{
    "content": "≤30 字，第一人称 IM 口语，从动作起手",
