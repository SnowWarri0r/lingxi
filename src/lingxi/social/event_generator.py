...
【关于 arc_id 怎么填】
一个 active arc 不是只在"重大节点"出现——它**染色**了这个人的日常。
事件只要是这个 arc 在生活里的具体表现/影响/触动，就填 arc_id。
判断标准：把这个事件从 NPC 的故事里抽走，arc 还讲得通吗？如果讲不通，标 arc_id。

以 arc='thesis_pressure'（论文压力）为例：
- "在实验室跑数据到凌晨" → arc_id='thesis_pressure'（论文工作）
- "跟导师 1on1 哭出来" → arc_id='thesis_pressure'（被 arc 推到边缘）
- "看到师兄朋友圈说论文中了，心情复杂" → arc_id='thesis_pressure'（同侪对比触动 arc）
- "在便利店买咖啡准备熬夜" → arc_id='thesis_pressure'（熬夜是为了赶 arc）
- "刷到一个搞笑猫视频笑了一下" → arc_id=null（纯路人插曲）
- "跟楼下大爷聊了两句天气" → arc_id=null（无关 arc）

以 arc='blind_date_pressure'（妈妈催相亲）为例：
- "妈妈又打电话问那个海归博士加微信没" → arc_id='blind_date_pressure'
- "看到邻居家儿子带女朋友回家，心里想自己女儿" → arc_id='blind_date_pressure'（同主题触动）
- "去超市买了把青菜" → arc_id=null
...
- 给每个事件评 significance（0.0-1.0）：
  ...
  * 0.9+：留给真正稀有的事（亲人病危/重大成就）
- content 要具体（"小敏点了麦当劳的炸鸡" 这种粒度）

输出格式：直接 JSON 数组，第一个字符是 '['。例：
[
  {{"type": "life", "content": "小敏在实验室跑数据到凌晨 2 点，跑出来还是 NaN", "significance": 0.4, "arc_id": "thesis_pressure"}},
  {{"type": "life", "content": "小敏刷到一个搞笑猫视频笑出声", "significance": 0.1, "arc_id": null}}
]
