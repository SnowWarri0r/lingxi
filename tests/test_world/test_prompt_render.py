"""Test that DailyBriefing renders into the inner-state section."""
...
class TestEmptyBriefingNoRender:
    def test_none_briefing_no_section(self):
        prompt = PromptBuilder(_persona()).build_system_prompt(
            inner_state=InnerState(),
            daily_briefing=None,
        )
        assert "今早扫到的事" not in prompt
...
class TestPopulatedRendering:
    def test_items_render_in_inner_state(self):
        ...
        assert "今早扫到的事" in prompt
        assert "JWST" in prompt
        assert "今天上海要下大雨" in prompt
        # Category tag included
        assert "[天文]" in prompt
        assert "[上海本地]" in prompt
    ...
    def test_caps_at_5_items(self):
        ...
        assert "voice-0" in prompt
        assert "voice-4" in prompt
        assert "voice-5" not in prompt
...
class TestStandaloneRender:
    def test_world_section_when_no_inner_state(self):
        ...
        assert "今早扫到" in prompt
        assert "今天上海要下雨" in prompt
