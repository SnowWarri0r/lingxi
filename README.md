...
改人设、改 prompt 拼装、换 responder 之后跑一遍：

```bash
lingxi-eval                      # 跑全部用例，跟基线比
lingxi-eval <case-id> --dump DIR # 跑一个，把 20 条回复落盘
lingxi-eval --baseline           # 认可当前结果，写成新基线
lingxi-eval --capture <recipient_key>  # 从线上状态生成用例骨架
```

一个用例把时钟、事实、对话历史全部冻住，**通过真实拼装管线重放**，采样 N 次，
用确定性判定器打分。冻结成品消息是测不到东西的——修改往往落在拼装层。

三条从踩坑里换来的规矩：

- **判定器先怀疑自己。** 有个用例第一版正则只命中 3/20，而人读下来 19/20 全错——
  失败的说法穷举不完。反过来想：正确回复必须包含什么？那一侧通常是封闭的
  （`regex_absent`）。一个报 3/20 的判定器会让任何"修好了"的结论都是假的。
- **阈值按实测分布定，不按直觉。** 定完不要随单轮结果上下挪；要动就重新采一批。
- **前提失效要报 BROKEN，不能报 PASS。** 悄悄失效的用例比没有用例更糟，
  它提供的是虚假的安全感。

用例和基线**不进版本库**——它们冻的是真人的对话原文和关于真人的事实，
跟 `data/` 是同一类东西。见 `.gitignore` 和 `evals/cases/example-case.yaml`。
